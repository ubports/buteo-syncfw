...
int SyncDBusAdaptor::status(int aAccountId, int &aFailedReason, QDateTime &aPrevSyncTime, QDateTime &aNextSyncTime)
{
    // handle method call com.meego.msyncd.status
    int out0;
    QMetaObject::invokeMethod(parent(), "status", Q_RETURN_ARG(int, out0),
            Q_ARG(int, aAccountId), Q_RETURN_ARG(int, aFailedReason), 
            Q_RETURN_ARG(QDateTime, aPrevSyncTime), Q_RETURN_ARG(QDateTime, aNextSyncTime));
    return out0;
}
